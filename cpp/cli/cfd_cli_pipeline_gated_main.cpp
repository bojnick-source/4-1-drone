/*
===============================================================================
Fragment 3.2.09 — CFD Gated Pipeline CLI (Manifest + Gate Summary + Corrected Outputs)
File: cfd_cli_pipeline_gated_main.cpp
===============================================================================
*/
...
int main() {
    try {
        ...
        cfg.pipeline.manifest_id = "cfd_run_002";
        cfg.pipeline.created_utc_iso8601 = "2026-01-03T00:00:00Z";
        cfg.pipeline.notes = "Gated calibration demo";
        ...
        // Disable GO/NO-GO thresholds in demo (recompute still enabled)
        ...
        const auto out = run_cfd_pipeline_gated(rows, gonogo, Tref, Pref, cfd_results, cfg);

        std::cout << "===== cfd_manifest.json =====\n" << out.base.manifest_json << "\n\n";
        std::cout << "===== cfd_manifest.csv =====\n"  << out.base.manifest_csv  << "\n\n";

        std::cout << "===== cfd_gate_summary =====\n";
        std::cout << "code=" << static_cast<unsigned>(out.gate_result.code)
                  << " msg=" << out.gate_result.message
                  << " total=" << out.gate_result.total
                  << " ok=" << out.gate_result.ok
                  << " rejected=" << out.gate_result.rejected
                  << " calibration_enabled=" << (out.calibration_enabled ? "true" : "false")
                  << "\n\n";

        if (out.calibration_enabled) {
            std::cout << "===== closeout_corrected.csv =====\n" << out.base.corrected_closeout_csv << "\n\n";
            std::cout << "===== gonogo_corrected.csv =====\n"   << out.base.corrected_gonogo_csv << "\n\n";
        }
